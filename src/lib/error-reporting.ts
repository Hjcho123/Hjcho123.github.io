...
export function reportRuntimeError(error: unknown, context: Record<string, unknown> = {}) {
  ...
  // editor telemetry never sees them. Forward to the runtime hook if present in
  // preview tooling. Loaders and server functions commonly throw a raw Response;
  // String(it) is the opaque "[object Response]", so pull out the status and URL instead.
  const message =
